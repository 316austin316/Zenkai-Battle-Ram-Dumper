...
def dump_ram(iteration, starting_address, upper_limit_address, ram_type):
    ...
    if matches:
        if ram_type == "vram":
            selected_matches = matches[:30]  # Select the first 3 matches for vram
        ...
        if selected_matches is not None:
            for i, match in enumerate(selected_matches):
                ...
                output_file = f"iteration_{iteration}/output_{ram_type}_{i}.bin"
                with open(output_file, "wb") as f:
                    f.write(found_data)
                    
                # Check the file size and delete if over 1GB
                if os.path.getsize(output_file) > 1 * 1024 * 1024 * 1024:  # 1GB in bytes
                    print(f"File {output_file} is over 1GB, deleting...")
...
def main():
    ...
    for iteration, (ioram_file, vram_file) in enumerate(zip(ioram_files, vram_files), start=1):
        ...
        if '05_tien_model6_ioram.pak.scz' in ioram_file and '05_tien_model6_vram.pak.scz' in vram_file:
        ...
        time.sleep(21)  # Adjust this as needed
        ...
        time.sleep(7)  # Adjust this as needed
